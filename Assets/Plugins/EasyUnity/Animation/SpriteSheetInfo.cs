...
public class SpriteSheetInfo : IEquatable<SpriteSheetInfo>
{
    ...
    public bool FirstNonBlank { get; set; }
    public bool DontBreakWhenBlank { get; set; }
    #endregion
    ...
    public SpriteSheetInfo(string assetPath)
    {
        ...
        FirstNonBlank = assetPath.Contains(".fnb");
        DontBreakWhenBlank = assetPath.Contains(".dbwb");
        ...
        if (parts.Last().Length.Within(1, 2))
        {
            nonDirectionPartsCount--;
            Direction = new AbsoluteDirection(parts.Last());
        }
        ...
        parts = mainParts[1].Remove(mainParts[1].IndexOfAny(new char[] {')', ']'})).Split(',', 'x');
        ...
        {
            parts = mainParts[2].Remove(mainParts[2].IndexOfAny(new char[] {')', ']'})).Split(',', 'x');
