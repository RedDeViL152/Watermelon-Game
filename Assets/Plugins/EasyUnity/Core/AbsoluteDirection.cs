...
public struct AbsoluteDirection : IEquatable<AbsoluteDirection>
{
    ...
    private static Vector2Int Validate(float x, float y, float step, float offset) => x == 0 && y == 0 ? Vector2Int.zero : Mathf.Atan2(y, x).InStepsOf(step, offset, true).RadToVector().RoundToInt();
